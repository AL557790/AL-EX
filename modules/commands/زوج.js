module.exports.config = {
    ...
    hasPermssion: 0,
    credits: "عمر",
    description: "زواج بالرد على رسالة",
    commandCategory: "ترفية",
    usages: "(يستخدم فقط بالرد على رسالة شخص)",
    cooldowns: 5,
    dependencies: {
        "axios": "",
        "fs-extra": "",
...
module.exports.run = async function ({ event, api }) {
    ...
    // 🔥 مهم: لازم يكون فيه Reply
    if (!event.messageReply)
        return api.sendMessage("⚠️ يجب الرد على رسالة شخص كي تعمل خاصية الزواج.", threadID, messageID);

    const two = event.messageReply.senderID; // ID الشخص لي رديت عليه
    const one = senderID;
