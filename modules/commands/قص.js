module.exports.config = {
    ...
    "credits": "DC-Nam",
    "hasPermission": 0,
    "description": "الإجابة على الأسئلة",
  ...
  module.exports.handleReaction = ({ api, event, handleReaction }) => {
    ...
    if (response == handleReaction.answer) {
        api.sendMessage("『 🌸 』  إجابتك صحيحة!!!", event.threadID, () => {
            setTimeout(function() {
                api.unsendMessage(handleReaction.messageID);
            }, 5000);
        });
    } else {
        api.sendMessage("『 🌸 』 إجابتك خاطئة!!!", event.threadID);
    }
  ...
  module.exports.run = async ({ api, event, args }) => {
    ...
    if (!fetch.data) return api.sendMessage("『 🌸 』 لم يتم العثور على سؤال بسبب انشغال الخادم", event.threadID, event.messageID);
    ...
    return request(encodeURI(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=ar&dt=t&q=${decode}`), (err, response, body) => {
        if (err) return api.sendMessage(" حدث خطأ ما!", event.threadID, event.messageID);
        ...
        return api.sendMessage(`『 🌸 』 إليك السؤال:\n- ${text}\n\n   👍: صحيح       😢: خاطئ`, event.threadID, async (err, info) => {
            ...
            if (data.answerYet !== 1) {
                api.sendMessage(`『 🌸 』 انتهى الوقت!! الجواب الصحيح هو ${fetch.data.results[0].correct_answer}`, event.threadID, info.messageID);
