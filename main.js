...
function getAllCommandFiles(dirPath, arrayOfFiles) {
    ...
    files.forEach(function(file) {
        ...
        if (fs.statSync(filePath).isDirectory()) {
            ...
            if (file.endsWith(".js") && !file.includes("example")) {
...
function onBot({ models: botModel }) {
    ...
    login(loginData, async(loginError, loginApiData) => {
        ...
        (function () {  
            // 📌 استبدلنا readdirSync بـ getAllCommandFiles
            const listCommand = getAllCommandFiles(global.client.mainPath + '/modules/commands')
                .filter(command => !global.config.commandDisabled.includes(path.basename(command)));
        ...
        // باقي الكود نفسه (تحميل events + تشغيل السيرفر إلخ)
        // 👇👇 ما غيرت فيه ولا شيء
        (function () {
...
app.get('/', (req, res) => {
  const htmlPath = path2.join(__dirname, 'index.html');
  fs2.readFile(htmlPath, 'utf8', (err, data) => {
    if (err) {
