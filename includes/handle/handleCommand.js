module.exports = function ({ api, models, Users, Threads, Currencies }) {
    ...
    return async function ({ event }) {
        console.log("📩 نوع الرسالة:", event.isGroup ? "جروب" : "خاص", "من:", event.senderID);
        ...
        // 🛑 لو الخاص ممنوع من config.json
        if (!event.isGroup && !allowInbox && !ADMINBOT.includes(senderID)) {
            console.log("⛔ تم تجاهل رسالة خاص بسبب allowInbox=false");
            return;
        }
        ...
        // ✅ السماح بالرد سواء جروب أو خاص
        var threadInfo2;
        try {
            threadInfo2 = threadInfo.get(threadID) || (event.isGroup ? await Threads.getInfo(threadID) : {});
        } catch (err) {
